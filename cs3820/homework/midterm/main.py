...
def simplify_added_year(entry) -> int:
    return int(entry.split(",")[1]) if type(entry) == str else nan

def simplify_duration(entry) -> float|int:
...
for service in movie_titles.values():
    for column, wrangler in zip(
        ("rating", "duration", "date_added"),
        (simplify_rating, simplify_duration, simplify_added_year)
    ):
...
# print(plus_18_across_services.loc["netflix"].loc[2017])
...
def linecharts():
    ...
    netflix_peak = plus_18_across_services.loc["netflix"].loc[2017]
    # plt.annotate(f"{netflix_peak}", (netflix_peak,2018), xycoords="data")
...
def save_plots():
    for plot in (barcharts, linecharts, histograms, scatterplots, piecharts):
        plot().savefig(f"./plots/{plot.__name__}.png")

save_plots()

# # Other plots and code

def netflix_linecharts():
    for age_rating in ("NR", "All", "All Children", "7+", "13+", "16+", "18+"):
        plt.plot(
            movie_titles["netflix"] \
                .loc[
                (movie_titles["netflix"].rating == age_rating) &
                (movie_titles["netflix"].type == "Movie"), "release_year"] \
                .value_counts() \
                .reindex(range(2003,2022)) \
                .sort_index(),
            label=age_rating,
            # color= "tab:red" if age_rating == "18+" else "gray"
        )
        # print(
        #     movie_titles["netflix"] \
        #         .loc[movie_titles["netflix"].rating == age_rating, "release_year"]\
        #         .value_counts() \
        #         .reindex(range(2003,2022)) \
        #         .sort_index()
        # )
    plt.gca().set_xlim(2005,2021)
    plt.gca().spines[["top","right"]].set_visible(False)
    plt.suptitle(
        "Netflix is adding less movies per year to their catalog",
        weight="bold"
    )
    plt.legend()
    plot = plt.gcf()
    plt.show()
    return plot
# netflix_linecharts().savefig(f"./plots/netflix_ratings_years.png")
# print(
#     movie_titles["netflix"].release_year.value_counts().sort_index()
#         )
# print(
#     movie_titles["amazon_prime"].date_added.value_counts().sort_index()
#         )
